...
plot_rho <- function(rho_val) {
  plot(rep(tau2, length(X_val)), unlist(rho_val), type = "n", ylim = 0:1,
       xlab = "proportion of infected treated",
       ylab = "relative decrease of R0",
       xaxs = "i", yaxs = "i")
...
```{r fig.width = 8, fig.height = 5.25, margin1 = FALSE, margin4 = TRUE}
mcx <- 3 / 2
opar <- par(mfrow = c(2, 3), mex = mcx, cex.axis = mcx, cex.lab = mcx, cex.main = mcx)
plot_rho(rho_val[[1]])
mtext("L = 20,000", 2, 2.5)
mtext("-log(Pe) = 1 / 3", 3, .5)
plot_rho(rho_val[[2]])
mtext("-log(Pe) = 1", 3, .5)
plot_rho(rho_val[[3]])
mtext("-log(Pe) = 3", 3, .5)
