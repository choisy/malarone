...
```{r margin1 = FALSE, margin3 = TRUE, fig.width = 5, fig.height = 5, eval = FALSE, include=FALSE}
# mosquito death rate (inverse of mosquito life expectancy):
g <- 1 / 14  # same unit as n (here in days)
# EPI:
n <- 7  # same unit as g (here in days)
...
tau2 <- seq2(0, 1)
rho3 <- function(X, D, r) 1 - tau2 * X * r * D * (1 - lambda_val)
X_val <- seq(0, 1, .1)
out <- map(X_val, rho3, D, r)
opar <- par(pty = "s")
plot(rep(tau2, length(X_val)), unlist(out), type = "n", ylim = 0:1,
     xlab = "proportion of infected that are treated",
     ylab = "relative decrease of R0",
     xaxs = "i", yaxs = "i")
abline(v = seq(0, 1, .05), col = "lightgrey")
abline(h = seq(0, 1, .05), col = "lightgrey")
abline(v = seq(0, 1, .1), col = "lightgrey", lwd = 2)
abline(h = seq(0, 1, .1), col = "lightgrey", lwd = 2)
box(bty = "o")
walk(out, lines2, x = tau2)
par(opar)
```
...
```{r fig.width = 8, fig.height = 6, margin1 = FALSE, margin3 = TRUE}
gn_val <- c(1 / 3, 1, 3)
L_val <- c(2, 6) * 1e4
