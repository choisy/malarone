...
L_val <- seq2(0, 100)
plotl(L_val, f(L_val),
      xlab = "oocysts load L", ylab = "infection probability f")
```
...
Combining `f()` with the observed distribution of oocyst loads in order to convert a
value of $\beta$ into a reduction factor of $b$ or $R_0$:

```{r}
f2 <- function(beta, prob = p_par, loads = 1:max_load) {
  densities <- dgeom(loads, prob)
  densities <- densities / sum(densities)
  map_dbl(beta, function(x) sum(densities * map_dbl(loads, ~ f((1 - x) * .x) / f(.x))))
}
```
