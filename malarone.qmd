...
### Anna and Thierry's idea

A function that computes $\rho$:

```{r}
X_val <- seq(0, 1, .1)
tau2 <- seq2(0, 1)

compute_rho <- function(gn, L, r, D) {
  K <- tau2 * r * D * (1 - lambda(D, L, 1, gn, nb = 1e6))
  map(X_val, ~ 1 - .x * K)
}
```

A function that plots the computed $\rho$ values as a function of the proportion $\tau$
of infected people that are treated:

```{r}
abline3 <- function(...) abline(..., col = "lightgrey")
abline4 <- function(...) abline3(..., lwd = lwd_val)

plot_rho <- function(rho_val, ylim = NULL, ylab = "relative decrease of R0",
                     hlines1 = seq(0, 1, .1), hlines2 = seq(0, 1, .05)) {
  if (is.null(ylim)) ylim <- 0:1
  plot(rep(tau2, length(X_val)), unlist(rho_val), type = "n", ylim = ylim, ylab = ylab,
       xlab = "proportion of infected treated", xaxs = "i", yaxs = "i")
  abline3(v = seq(0, 1, .05))
  abline3(h = hlines2)
  abline4(v = seq(0, 1, .1))
  abline4(h = hlines1)
  box(bty = "o")
  walk(rho_val, lines2, x = tau2)
}
```

Computing the $\rho$ values (takes about 8"):

```{r eval = FALSE}
gn_val <- c(1 / 3, 1, 3)
L_val <- c(2, 6) * 1e4
rho_val <- expand.grid(gn_val, L_val) |>
  with(map2(Var1, Var2, compute_rho, r = 1 / (14 * 24), D = 28 * 24))
```
...
if (file_exists("rho_val.rds")) {
  rho_val <- readRDS3("rho_val.rds")
} else {
  rho_val <- expand.grid(gn_val, L_val) |>
    with(map2(Var1, Var2, compute_rho, r = 1 / (14 * 24), D = 28 * 24))
  saveRDS2(rho_val, "rho_val.rds")
}
```

Plotting the $\rho$ values for various values of $-\log(P_e)$ and sporozoite load $L$.
As a reminder, $P_e = e^{-gn}$ is the probability that an infected mosquito becomes
infectious, with $g$ the mosquito death rate and $n$ the EIP.

```{r}
plot_multipanel <- function(x, gn = paste("-log(Pe) =", c("1 / 3", "1", "3")),
                            ylim = NULL, ylab = bquote(R[0] ~ .("reduction factor")),
                            hlines1 = seq(0, 1, .1), hlines2 = seq(0, 1, .05)) {
  if (is.null(ylim)) ylim <- 0:1
  mcx <- 3 / 2
  plot_rho2 <- function(y) plot_rho(y, ylim, ylab, hlines1, hlines2)
  op <- par(mfrow = c(2, 3), mex = mcx, cex.axis = mcx, cex.lab = mcx, cex.main = mcx)
  plot_rho2(x[[1]])
  mtext("L = 20,000", 2, 2.5)
  mtext(gn[1], 3, .5)
  plot_rho2(x[[2]])
  mtext(gn[2], 3, .5)
  plot_rho2(x[[3]])
...
plot_multipanel(rho_val, c("EPI = e(0) / 3", "EPI = e(0)", "EPI = 3 x e(0)"))
```

Let's try a different range of the EPI compared to the mosquito life expectancy:

```{r eval = FALSE}
gn_val2 <- c(1 / 2, 1, 2)
rho_val2 <- expand.grid(gn_val2, L_val) |>
  with(map2(Var1, Var2, compute_rho, r = 1 / (14 * 24), D = 28 * 24))
```
...
```{r fig.width = 8, fig.height = 5.25, margin1 = FALSE, margin3 = TRUE}
plot_multipanel(rho_val2, c("EPI = e(0) / 2", "EPI = e(0)", "EPI = 2 x e(0)"))
```

And an even narrower range:
...
```{r fig.width = 8, fig.height = 5.25, margin1 = FALSE, margin3 = TRUE}
plot_multipanel(rho_val3, c("EPI = 0.4 x e(0)", "EPI = 0.7 x e(0)", "EPI = e(0)"))
```


### With $R_0$ values

If we want to look at some specific value of $R_0$, we first need to acknowledge that,
every else being constant, $R_0$ is proportional to $P_e$:

$$
R_0 \propto P_e = e^{-gn}
$$
With this being considered, let's now consider for example a reference value of
$R_0=15$ in absence of treatment and when the EPI is equal to the mosquito life
expectancy:

```{r fig.width = 8, fig.height = 5.25, margin1 = FALSE, margin3 = TRUE}
rho_val2b <- rho_val
rho_val2b[c(1, 4)] <- map(rho_val2[c(1, 4)],
                          function(x) map(x, ~ .x * 15 * exp(1 - 1 / 2)))
rho_val2b[c(2, 5)] <- map(rho_val2[c(2, 5)], function(x) map(x, ~ .x * 15))
rho_val2b[c(3, 6)] <- map(rho_val2[c(3, 6)],
                          function(x) map(x, ~ .x * 15 * exp(1 - 2)))
plot_multipanel(rho_val2b, c("EPI = e(0) / 2", "EPI = e(0)", "EPI = 2 x e(0)"),
                c(0, 30), bquote(R[0]), seq(5, 25, 5), 1:29)
```

A mass treatment programme in which everybody in the human population takes malarone
every $D$ days (and in absence of synchrony between individuals) would lead to an $R_0$
reduction factor equal to $\lambda$ as defined in @eq-lambda:
...
```{r}
system.time(
out <- expand.grid(c(2e4, 6e4), c(.5, 1, 2)) |> 
  with(map2(Var1, Var2, lambda, D = 28 * 24, g = 1, nb = 1e6))
)
```
