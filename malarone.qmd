...
#lambda(D = 14 * 24, L = 6e4, g = 1 / 14, n = 7)
...
$$
\Delta(\bar{X}, R_0, \tau |\theta) = \frac{(1 - \bar{X})(1 - \rho(\bar{X}, \tau|\theta))R_0}{((1-(1-\rho(\bar{X}, \tau|\theta))\bar{X})R_0 - 1)\tau}
$$

with $\theta = \{g,n,L,r,D\}$. The corresponding R code:

```{r}
Delta <- function(D, L, g, n, X, tau, r, R0) {
  rho_val <- rho(D, L, g, n, X, tau, r)
...
out <- map(c(5, 10, 15), ~ map_dfc(X_val, Delta2, .x, D, tau, r))
```
...
persp2 <- function(x, y, z, ...) {
  persp(x, y, as.matrix(z), theta = 60, zlim = c(0, .15), col = NA, ...)
}
persp3 <- function(...) persp2(..., axes = FALSE, box = FALSE)
persp3(tau, X_val, out[[1]], border = 2); par(new = TRUE)
persp3(tau, X_val, out[[2]], border = 3); par(new = TRUE)
persp2(tau, X_val, out[[3]], border = 4,
       xlab = "treatment rate tau", ylab = "prevalence X",
       zlab = "number of infections prevented per personne treated")
```
