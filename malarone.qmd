...
```{r}
pi1 <- function(D, g, n, mu) mu * (1 - lambda(D, g, n))
```
...
```{r}
rho1 <- function(D, g, n, X, tau, r) {
  1 - (1 - exp(- tau * X * r * D)) * (1 - lambda(D, g, n))
}
```
...
compute_lambda_val <- function(n, g, D) {
  tidyr::expand_grid(n, g, D) |> 
    mutate(lambda = pmap_dbl(list(g, n, D), ~ lambda(..1, ..2, ..3)))
}
```

Let's now compute $\lambda$ values (takes ca 50" on the laptop):
...
```{r}
lambda_vals2 <- lambda_vals |> 
  dplyr::select(n, g, D, lambda) |> 
  arrange(n, g, D, lambda)
...
plot(NA, xlim = c(30, 0), ylim = 0:1, xaxt = "n", xaxs = "i", yaxs = "i",
     xlab = "intake frequency (/day)", ylab = ylab_pi)
abline(v = ats, col = "grey")
abline(h = prev, col = "grey")
axis(1, c(ats, 1), c(paste0("1/", ats), 1))
with(tmp,
     walk2(prev,
           YlOrRd11, ~ lines(D / 24, (1 - (1 - min_of_following(lambda)) * .x),
                             col = .y, lwd = lwd_val)))
```
...
#}
#
#if (file_exists("lambda_vals3.rds")) {
#  lambda_vals3 <- readRDS3("lambda_vals3.rds")
#} else {
#  lambda_vals3 <- compute_lambda_val(D_val = c(seq(1, 7, .1), 8:28) * 24,
#                                     n_val = c(6, 10, 14),
#                                     L_val = c(1, 5, 10) * 1e4,
#                                     g = 1 / 7)
#  saveRDS2(lambda_vals3, "lambda_vals3.rds")
}
```
...
```{r}
plot_mass_treatment2 <- function(x, y, x_axis = TRUE, y_axis = TRUE, ylim = 0:1,
                                xlab = "intake frequency (every D days)",
                                ylab = ylab_pi, hlines = seq(0, 1, .1), transform = 1,
                                xmin = 21, xmax = 56, xby = 7) {
  plot(rev(x), y, xlim = c(xmax, xmin), ylim = ylim, type = "n",
       xlab = xlab, ylab = ylab, xaxs = "i", yaxs = "i", axes = FALSE)
  box(bty = "o")
  if (x_axis) axis(1, seq(xmin, xmax, xby))
  if (y_axis) axis(2)
...
```{r}
multipanels_mass2 <- function(lambda_vals, ylim = 0:1,
                              ylab = ylab_pi,
                              hlines = seq(0, 1, .1), R0o = NULL) {
  params <- attr(lambda_vals, "params")
  ...
  plot_mass_treatment2b <- function(y, ylob = ylab, ...) {
    plot_mass_treatment2(params$D_val / 24, y, ylim = ylim, ylab = ylob,
                        hlines = hlines, ...)
  }
  plot_mass_treatment3b <- function(y, ...) {
    plot_mass_treatment2b(y, xlab = NA, x_axis = FALSE, ...)
  }
  plot_mass_treatment4b <- function(y, ...) {
    plot_mass_treatment2b(y, ylob = NA, y_axis = FALSE, ...)
  }
  plot_mass_treatment5b <- function(y, ...) {
    plot_mass_treatment4b(y, xlab = NA, x_axis = FALSE, ...)
  }
  
  opar <- parmfrow3x3()
  plot_mass_treatment3b(lambda_vals[[7]], transf = trnsfrm[1])
  mtext2(params$n_val[1])
  plot_mass_treatment5b(lambda_vals[[8]], transf = trnsfrm[2])
  mtext2(params$n_val[2])
  legend5("bottomleft", title = "proportion treated", text.width = .5, 
          legend = paste(seq(0, 100, 10), "%"))
  plot_mass_treatment5b(lambda_vals[[9]], transf = trnsfrm[3])
  mtext2(params$n_val[3])
  mtext3(params$L_val[3])
  plot_mass_treatment3b(lambda_vals[[4]], transf = trnsfrm[1])
  plot_mass_treatment5b(lambda_vals[[5]], transf = trnsfrm[2])
  plot_mass_treatment5b(lambda_vals[[6]], transf = trnsfrm[3])
  mtext3(params$L_val[2])
  plot_mass_treatment2b(lambda_vals[[1]], transf = trnsfrm[1])
  plot_mass_treatment4b(lambda_vals[[2]], transf = trnsfrm[2])
  plot_mass_treatment4b(lambda_vals[[3]], transf = trnsfrm[3])
  mtext3(params$L_val[1])
  par(opar)
}
```
...
if (file_exists("rho_val2.rds")) {
  rho_val2 <- readRDS3("rho_val2.rds")
} else {
  rho_val2 <- compute_rho_val(X_val = seq(0, 1, .1),
                              tau_val = seq2(0, 1),
                              n_val = c(6, 10, 14),
                              ...
                              r = 1 / (14 * 24),
                              nb = 1e6)
  saveRDS2(rho_val2, "rho_val2.rds")
}

if (file_exists("rho_val3.rds")) {
  rho_val3 <- readRDS3("rho_val3.rds")
} else {
  rho_val3 <- compute_rho_val(X_val = seq(0, 1, .1),
                              tau_val = seq2(0, 1),
                              n_val = c(6, 10, 14),
                              ...
                              r = 1 / (14 * 24),
                              nb = 1e6)
  saveRDS2(rho_val3, "rho_val3.rds")
}
```
...
```{r supfig_infc1, fig.width = 8, fig.height = 7.875, margin1 = FALSE, margin3c = TRUE}
multipanels_infected(rho_val1)
```

```{r supfig_infc2, fig.width = 8, fig.height = 7.875, margin1 = FALSE, margin3c = TRUE}
multipanels_infected(rho_val2)
```

```{r supfig_infc3, fig.width = 8, fig.height = 7.875, margin1 = FALSE, margin3c = TRUE}
multipanels_infected(rho_val3)
```
