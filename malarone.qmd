...
```{r}
#system.time(tmp <- lambda(D = 28 * 24, g = 1 / 14, n = 7))
```
...
```{r}
compute_lambda_val <- function(D_val, n_val, g) {
  out <- with(expand.grid(n_val, D_val), map2(Var1, Var2, ~ lambda(.y, g, .x)))
  attr(out, "params") <- list(D_val = D_val, n_val = n_val, g = g)
  out
}
```
...
```{r}
system.time(lambda(D = 3 * 24, g = 1 / 14, n = 10))
```

```{r}
#lambda(D = 3 * 24, g = 1 / 14, n = 10)
```


```{r}
system.time(
compute_lambda_val(D_val = (3:4) * 24,
                                  n_val = c(6, 10, 14),
                                  g = 1 / 14)
)
```
