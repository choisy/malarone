...
colors_lines_10 <- hcl.colors(10, "YlOrRd", rev = TRUE)
...
x_val <- seq2(0, 1)
walk2(1:10 * 1e4, colors_lines_10,
      ~ lines(x_val, f(x_val * .x) / f(.x), col = .y, lwd = lwd_val))
box(bty = "o")

legend2 <- function(...) {
  legend(title = "sporozoite load L", bty = "n", col = rev(colors_lines_10), lty = 1,
...
```{r}
compute_lambda_val <- function(D_val, n_val, L_val) {
  out <- with(expand.grid(n_val, L_val),
              map2(Var1, Var2, ~ map_dbl(D_val, lambda, L = .y, g = 1 / 14, n = .x)))
  attr(out, "params") <- list(D_val = D_val, n_val = n_val, L_val = L_val)
  out
}
```
...
A function that plot the mass treatment:

```{r}
colors_lines_11 <- hcl.colors(11, "YlOrRd", rev = TRUE)

#plot_mass_treatment <- function(x, y, x_axis = TRUE, y_axis = TRUE,
#                                xlab = "nb of days D between intakes",
#                                ylab = bquote(R[0] ~ "reduction factor" ~ pi)) {
#  plot(x, y, xlim = c(0, 30), ylim = 0:1, type = "n",
...
#  walk2(seq(0, 1, .1), colors_lines_11,
#        ~ lines(x, 1 - (1 - min_of_following(y)) * .x, col = .y, lwd = lwd_val))
#}
...
plot_mass_treatment <- function(x, y, x_axis = TRUE, y_axis = TRUE, ylim = 0:1,
                                ...
                                ylab = bquote(R[0] ~ "reduction factor" ~ pi),
                                hlines = seq(0, 1, .1)) {
  plot(x, y, xlim = c(0, 30), ylim = ylim, type = "n",
       xlab = xlab, ylab = ylab, xaxs = "i", yaxs = "i", axes = FALSE)
  box(bty = "o")
  if (x_axis) {axis(1, seq(0, 30, 5)); axis(1, c(15, 25))}
  if (y_axis) axis(2)
...
#  plot_mass_treatment2(y, ylab = NA, y_axis = FALSE, ...)
#}
...
#
#mcx <- 3 / 2
#opar <- par(mfrow = c(3, 3), mex = mcx, cex.axis = mcx, cex.lab = mcx, cex.main = mcx)
#
#plot_mass_treatment3(lambda_vals[[7]])
#mtext2(params$n_val[1])
#
#colors_lines_11 <- hcl.colors(11, "YlOrRd", rev = TRUE)
#par(xpd = TRUE)
#legend("bottomright", title = "proportion treated", lty = 1, lwd = 2, adj = c(1, .5),
#       col = colors_lines_11, inset = c(-.15, -.23), x.intersp = 4, text.width = .5, 
#       legend = paste(seq(0, 100, 10), "%"))
#par(xpd = FALSE)
#
#plot_mass_treatment5(lambda_vals[[8]])
#mtext2(params$n_val[2])
#plot_mass_treatment5(lambda_vals[[9]])
#mtext2(params$n_val[3])
#mtext3(params$L_val[3])
#
#plot_mass_treatment3(lambda_vals[[4]])
#plot_mass_treatment5(lambda_vals[[5]])
#plot_mass_treatment5(lambda_vals[[6]])
#mtext3(params$L_val[2])
#
#plot_mass_treatment2(lambda_vals[[1]])
#plot_mass_treatment4(lambda_vals[[2]])
#plot_mass_treatment4(lambda_vals[[3]])
...
plot_treatment_infected <- function(x, y, x_axis = TRUE, y_axis = TRUE, ylim = 0:1,
  ...
  abline3(h = hlines2)
  box(bty = "o")
  walk2(y, colors_lines_11, ~ lines(x, .x, col = .y, lwd = lwd_val))
}
```
...
plot_multipanels <- function(rho_val, ylim = 0:1,
  ...
  plot_treatment_infected3(rho_val[[7]])
  mtext2(params$n_val[1])
  
  par(xpd = TRUE)
  legend("bottomright", title = "prevalence", lty = 1, lwd = 2, adj = c(1, .5),
         col = colors_lines_11, inset = c(-.07, -.22), x.intersp = 4, text.width = 0, 
         legend = paste(seq(0, 100, 10), "%"))
  par(xpd = FALSE)
  
  plot_treatment_infected5(rho_val[[8]])
  mtext2(params$n_val[2])
  plot_treatment_infected5(rho_val[[9]])
  mtext2(params$n_val[3])
  mtext3(params$L_val[3])
  
  plot_treatment_infected3(rho_val[[4]])
  plot_treatment_infected5(rho_val[[5]])
  plot_treatment_infected5(rho_val[[6]])
  mtext3(params$L_val[2])
  
  plot_treatment_infected2(rho_val[[1]])
  plot_treatment_infected4(rho_val[[2]])
  plot_treatment_infected4(rho_val[[3]])
  mtext3(params$L_val[1])
...
```{r fig.width = 8, fig.height = 7.875, margin1 = FALSE, margin3c = TRUE}
#ymax <- 10
#plot_multipanels(transformation(lambda_val, 5), ylim = c(0, ymax), ylab = bquote(R[0]),
#                 hlines1 = 0:ymax, hlines2 = seq(0, ymax, .5))
```

```{r fig.width = 8, fig.height = 7.875, margin1 = FALSE, margin3c = TRUE}
ymax <- 10
plot_multipanels(transformation(rho_val1, 5), ylim = c(0, ymax), ylab = bquote(R[0]),
                 hlines1 = 0:ymax, hlines2 = seq(0, ymax, .5))
```
...
legend("bottomright", title = "proportion treated", lty = 1, lwd = 2, adj = c(1, .5),
       ...
       col = c(colors_lines_11[1:3], rep("#FFFFFF", nb_mid), colors_lines_11[10:11]), 
       legend = c(leg[1:3], rep(".", nb_mid), leg[10:11]))
par(xpd = FALSE)
mtext("a", line = -.5, at = -4.7, cex = 2, font = 2)

plot_treatment_infected(params2$tau, rho_val1[[5]])
par(xpd = TRUE)
legend("bottomleft", title = "prevalence", lty = 1, lwd = 2, adj = c(1, .5),
       col = colors_lines_11, x.intersp = 4, text.width = 0,
       legend = paste(seq(0, 100, 10), "%"))
par(xpd = FALSE)
