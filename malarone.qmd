...
tmp <- lambda_vals |>
  filter(D > 27 * 24) |> 
  tidyr::expand_grid(X = prev, tau = seq2(0, 1)) |> 
...
```{r eval = FALSE}
rho1(D = 28 * 24, g = 1 / 14, n = 7, X = .1, tau = .9, r = 1 / (14 * 24))
```
...
if (file_exists("rho1_val.rds")) {
  ...
  (rho1_val <- rho1(D = 28 * 24, g = 1 / 14, n = 7, X = .1, tau = .9,
                    r = 1 / (14 * 24)))
...
```{r}
compute_rho_val <- function(n, g, X, tau, D, r) {
  tidyr::expand_grid(n, g, X, tau) |> 
    mutate(rho = pmap_dbl(list(g, n, X, tau), ~ rho1(D, ..1, ..2, ..3, ..4, r)))
}
```

```{r}
#compute_rho_val(n = c(6, 10, 14), g = 1 / c(7, 10, 14), X = prev, tau = prev, D = 28 * 24, r = 1 / (14 * 24))
system.time(
out <- compute_rho_val(n = c(6, 10, 14), g = 1 / c(7, 10, 14), X = prev, tau = c(.3, .6), D = 28 * 24, r = 1 / (14 * 24))
)
```


```{r}
rho_vals <- compute_rho_val(X_val = seq(0, 1, .1),
                            tau_val = seq2(0, 1),
                            n_val = c(6, 10, 14),
                            D = 28 * 24,
                            g = 1 / 14,
