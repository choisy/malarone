...
```{r}
prev <- 0:10 / 10
pi_vals <- pi(lambda_vals, prev)
```

```{r}
#make_grid1 <- function(v, h) {
#  function() {
#    abline(v = v, col = "grey")
#    abline(h = h, col = "grey")
#  }
...
plot_pi_D <- function(x, xlab = "intake frequency (/day)", ylab = ylab_pi,
                      ylim = 0:1, plot_grid = make_grid1(ats, prev)) {
  plot(NA, xlim = c(30, 0), ylim = ylim, axes = FALSE,
...
plot_mass_treatment <- function(x, ylim = 0:1,
                                plt_grd = make_grid1(ats, prev), ylab = ylab_pi) {
  plot_pi_D0 <- function(...) plot_pi_D(..., ylim = ylim, plot_grid = plt_grd)
  plot_pi_D_ul <- function(...) plot_pi_D0(..., xlab = NA, ylab = ylab)
  plot_pi_D_ur <- function(...) plot_pi_D0(..., xlab = NA, ylab = NA)
  plot_pi_D_lr <- function(...) plot_pi_D0(..., ylab = NA)
  
  opar <- par(mfrow = c(3, 3))
  plot_pi_D_ul(filter(x, n ==  6, g <  .1))
  mtext2(6); axis(2)
  plot_pi_D_ur(filter(x, n ==  6, g == .1))
  mtext2(10)
  legend5("bottomleft", title = "proportion treated", text.width = .5,
          legend = paste(seq(0, 100, 10), "%"))
  plot_pi_D_ur(filter(x, n ==  6, g >  .1))
  mtext2(14); mtext3(14)
  plot_pi_D_ul(filter(x, n == 10, g <  .1))
  axis(2)
  plot_pi_D_ur(filter(x, n == 10, g == .1))
  plot_pi_D_ur(filter(x, n == 10, g >  .1))
  mtext3(10)
  plot_pi_D0(filter(x, n == 14, g <  .1), ylab = ylab)
  axis1(); axis(2)
  plot_pi_D_lr(filter(x, n == 14, g == .1))
  axis1()
  plot_pi_D_lr(filter(x, n == 14, g >  .1))
  axis1(); mtext3(7)
  par(opar)
...
```{r}
plot_treatment <- function(x, f = plot_pi_D, ylim = 0:1, axis1fct = axis1,
                           add_lgd = make_legend(title = "proportion treated",
                                                 text.width = .5),
                           ablnv, ablnh,
                           plt_grd = make_grid1(ats, prev), ylab = ylab_pi) {
  f0 <- function(...) f(..., ylim = ylim, ablinev = ablnv, ablineh =  ablnh)
...
plot_mass_treatment <- function(x, ...) {
  plot_treatment(x,
                 f = plot_pi_D,
                 axis1fct = axis1,
                 add_lgd = make_legend(title = "proportion treated", text.width = .5),
                 plt_grd = make_grid1(ats, prev), ...)
}
```
...
plot_mass_treatment <- function(x, ...) {
  plot_treatment(x,
                 f = plot_pi_D,
                 axis1fct = function() axis(1, c(ats, 1), c(paste0("1/", ats), 1)),
                 add_lgd = function() legend6(title = "proportion treated",
                                              text.width = .5),
                 ablnv = function() abline(v = ats, col = "grey"), ...)
}
```
...
plot_mass_treatment(pi_vals, ylim = 0:1, ylab = ylab_pi,
...
plot_tau_rho <- function(x, ylab = ylab_rho,
                         xlab = bquote(atop("Proportion" ~ tau ~ "of new cases",
                                            "that are treated (%)")),
                         ylim = 0:1, ablinev, ablineh) {
  plot(NA, xlim = 0:1, ylim = ylim, xlab = xlab, ylab = ylab, axes = FALSE)
  ablinev()
  ablineh()
  box(bty = "o")
  x |> 
...
```{r fig.width = 8, fig.height = 7.875, margin1 = FALSE, margin3c = TRUE, include=FALSE, eval=FALSE}
opar <- par(mfrow = c(3, 3))
...
The effect of mass treatment would look like:

```{r fig.width = 8, fig.height = 7.875, margin1 = FALSE, margin3c = TRUE}
pi_vals |>
  R0_effect() |> 
  plot_mass_treatment(ylim = c(0, 10), ylab = bquote(R[0]),
                      ablnh = function() abline(h = 1:9, col = "grey"))
```

And the effect of the treatment of infected would like:
