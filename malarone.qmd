...
@aleshnick_experimental_2020 and 1 of @miura_strong_2019 and that we reproduce here:

```{r}
all_data <- read.table(paste0(path2data, "ASAAP4b_oocyst2.txt"), header = TRUE)

oocysts_control <- all_data |> 
  filter(day_of_dissection == "7dpi",
         time_points %in% c("Control", "Unfed"),
         total_oocysts > 0) |> 
  select(total_oocysts) |> 
  na.exclude()

oocysts_control |> 
  pull(total_oocysts) |> 
  hist(main = NA, xlab = "oocyst load L in the controls", ylab = "frequency")
...
aleshnick2020 <- path2data |> 
  ...
  select(load, infection)

mod <- gam(infection ~ s(load), binomial, aleshnick2020, method = "REML")
x <- seq(1, max(oocysts_control$total_oocysts) * 1000, le = 512)
x_ooc <- x / 1000
...
polygon(c(x_ooc, rev(x_ooc)), c(predictions$ul, rev(predictions$ll)), border = NA, col = adjustcolor(4, .2))
...
```{r}
L_val <- seq2(0, max(oocysts_control$total_oocysts))
plotl(L_val, f(L_val),
      xlab = "oocysts load L", ylab = "infection probability f")
```
...
beta_val <- .5
load_val <- 30
f((1 - beta_val) * load_val) / f(load_val)
```
