...
Figure 2B from @aleshnick_experimental_2020 suggests a non-linear relationship
...
effect of $\beta$ on $b$ can thus be modelled as so:
...
$f$ is the functional relationship between the oocsyts load $L$ and the
...
$$
\sum_N\frac{f((1 - \beta(t))L)}{f(L)}b \omega(L)
$$ {#eq-TRA_mean}
...
And this is what this function $f_0$ function coded in `f0()` looks like:

```{r}
L_val <- seq2(0, 100)
plotl(L_val, f0(L_val),
      xlab = "oocysts load L", ylab = "infection probability f0")
```
...
Combining `f0()` with the observed distribution of oocyst loads in order to convert a
...
f <- function(beta, prob = p_par, loads = 1:max_load) {
  ...
  map_dbl(beta, \(x) sum(densities * map_dbl(loads, ~ f0((1 - x) * .x) / f0(.x))))
}
```
